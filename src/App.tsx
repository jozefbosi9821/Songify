...
type HistoryItem = {
  view: 'home' | 'search' | 'settings' | 'playlist' | 'library' | 'artist' | 'profile' | 'downloaded';
  playlistId?: string;
  artistName?: string;
};
...
function AppContent() {
  ...
  const [history, setHistory] = useState<HistoryItem[]>([{ view: 'home' }]);
  ...
  const handleGoBack = () => {
    if (historyIndex > 0) {
      ...
      if (item.playlistId) {
        setCurrentPlaylistId(item.playlistId);
  ...
  const handleNavigate = (view: 'home' | 'search' | 'settings' | 'playlist' | 'library' | 'artist' | 'profile' | 'downloaded', extra?: string) => {
    setCurrentView(view);
    ...
    // Update history
    const newItem: HistoryItem = { view };
    if (playlistId) newItem.playlistId = playlistId;
    if (artistName) newItem.artistName = artistName;
  ...
  return (
    <div className={`h-screen flex flex-col overflow-hidden transition-colors duration-300 ${theme}`} style={{ backgroundColor: 'var(--bg-main)', color: 'var(--text-main)' }}>
      <TitleBar
        ...
        onSearchFocus={() => handleNavigate('search')}
        username={username}
        ...
        <div className="w-[280px] h-full">
          <Sidebar 
            onNavigate={(view, playlistId) => {
              handleNavigate(view, playlistId);
            }}
