...
const SERVICE_NAME = 'Songify';
ipcMain.handle('secure-storage-set', async (event: any, key: string, value: string) => {
  try {
...
ipcMain.handle('secure-storage-get', async (event: any, key: string) => {
  try {
...
ipcMain.handle('secure-storage-delete', async (event: any, key: string) => {
  try {
